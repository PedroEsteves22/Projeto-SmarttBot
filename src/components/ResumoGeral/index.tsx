...
import { AlignLeft, AlignRight, LoadingBox, LoadingSquare, MovimetacaoTotal, PapeisNegociados, PapeisNegociadosContainer, PapeisNegociadosContent, ResumoContent } from "./styles";
...
export function ResumoGeral() {
  ...
  const { listaAtivos, atualizarAtivos, loading, loadingExibirTelaAtivo } = useContext(ListaAtivosContext);
  const [ resumoMovimentacao, setResumoMovimentacao ] = useState(0);
  const [ totalTransacao, setTotalTransacao ] = useState(0);
  const [ loadingScreen, setLoadingScreen ] = useState(true);

  useEffect(() => {
    if(loadingExibirTelaAtivo) {
      setLoadingScreen(false);
    } else {
      setLoadingScreen(true);
    }
  }, [loadingExibirTelaAtivo])
